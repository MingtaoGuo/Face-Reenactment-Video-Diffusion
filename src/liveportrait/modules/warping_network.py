    ...
    def forward(self, feature_3d, kp_driving, kp_source):
        if self.dense_motion_network is not None:
            ...
            if self.flag_use_occlusion_map and (occlusion_map is not None):
                ...
                # import numpy as np
                # Image.fromarray(np.uint8(occlusion_map.cpu().numpy()[0, 0]*255)).save("out.png")
                # print("occlusion_map: ", occlusion_map.shape, occlusion_map.min(), occlusion_map.max())
                # print("deformation: ", deformation.shape, deformation.min(), deformation.max())
                # print("feature_3d: ", feature_3d.shape, feature_3d.min(), feature_3d.max())
        ret_dct = {
