...
from ..modules.warping_feature_mapper import WarpingFeatureMapper 
...
@dataclass
class LargePosePipelineOutput(BaseOutput):
    r"""
    Output class for LargePose pipeline.
...
class LargePosePipeline(DiffusionPipeline):
    r"""
    ...
    def _encode_pose_identity(
        self, 
        source_image: torch.Tensor, 
        driving_image: torch.Tensor,
        do_classifier_free_guidance: bool,
    ):
        # Get latents_head
        pose_identity_latent = self.motion_extractor(source_image, driving_image).to(dtype=source_image.dtype)  # [b, f, 256, 64, 64]
        head_latents = self.head_embedder(pose_identity_latent)          # [b, f, 320, h, w]
        ...
        if do_classifier_free_guidance:
            ...
            # Here we concatenate the unconditional and text embeddings into a single batch
            # to avoid doing two forward passes
            head_latents = torch.cat([negative_head_latents, head_latents]) # no need to cfg
    ...
    def __call__(
        ...
        image_ref: Union[torch.FloatTensor],
        image_drv: Union[torch.FloatTensor],
        height: int = 576,
        width: int = 1024,
        ...
        head_latents = self._encode_pose_identity(
            image_ref, image_drv, do_classifier_free_guidance=self.do_classifier_free_guidance,
        )
        ...
        guidance_scale = torch.linspace(min_guidance_scale, max_guidance_scale, num_frames).unsqueeze(0).flip([1])
        ...
        return LargePosePipelineOutput(frames=frames)
